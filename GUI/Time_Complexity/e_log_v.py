...
complexity = E * np.log(V)

# Create the figure for 3D plotting
fig = plt.figure(figsize=(5, 6), facecolor='black')  # Set figure size and background color
ax = fig.add_subplot(111, projection='3d')

# Plot E (x-axis), V (y-axis), and complexity (z-axis)
ax.plot(E, V, complexity, color='blue', marker='o', markersize=5, label='O(E log V)')

# Set axis labels and title
ax.set_title('3D Graph of O(E log V)', color='white')
ax.set_xlabel('Number of Edges (E)', color='white')
ax.set_ylabel('Number of Vertices (V)', color='white')
ax.set_zlabel('Time Complexity O(E log V)', color='white')
